let simulatedUnreadCount = 3; // Giả sử có 3 thông báo chưa đọc
...
function navigateTo(event, page) {
    ...
    if (page === 'home') {
        alert('Navigating to home (simulated)');
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!carStatusElement) {
        console.error('Không tìm thấy phần tử #car-status trong HTML');
    }
    if (!orderButton) {
    ...
    const userId = userData.UserID || userData.userID; // cẩn thận case
    ...
    const carId = urlParams.get('id') || '2';
    ...
    fetch(`${API_BASE_URL}/CarDetail/${carId}`, {
        headers: {
            ...
            // Loại bỏ 'Role': 'Admin' để kiểm tra
        }
    })
        .then(response => {
            console.log('Phản hồi API CarDetail:', response.status, response.statusText);
            if (!response.ok) {
                console.error('Lỗi API CarDetail chi tiết:', response.status, response.statusText);
        ...
        .catch(error => {
            ...
            console.log('Nút Place Order được kích hoạt mặc dù có lỗi API để kiểm tra chuyển hướng');
            orderButton.disabled = false;
            orderButton.classList.remove('disabled');
            const placeOrderUrl = `../PlaceOrder/index.html?carModelId=${carId}`;
            if (orderButton.tagName.toLowerCase() === 'a') {
                orderButton.href = placeOrderUrl;
                orderButton.addEventListener('click', (e) => {
                    console.log('Nút Place Order (a) được nhấn, chuyển hướng đến:', placeOrderUrl);
